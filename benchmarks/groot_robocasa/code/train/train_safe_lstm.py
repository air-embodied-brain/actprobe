"""Train SAFE-LSTM per Protocol §4.1.

Adam lr=3e-4, wd=1e-4, batch=512, 1000 epochs.
Per-step BCE with uniform target (failure as positive class).
Class-frequency weighting.

Usage:
  source env.sh
  python code/train/train_safe_lstm.py --split allseen --seed 0
"""
import argparse
import random
import sys
...
from lib.methods.safe_lstm import SafeLSTM

DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
CKPT_DIR = ROOT / "checkpoints" / "safe_lstm"
...
def train(splits, seed):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, lab, succ, lens in tr_ld:
            ...
            scores = model(x, lens)   # (B, T) sigmoid in [0,1]
            targets = (1 - succ).unsqueeze(1).expand_as(scores).float()  # 1 = failure

            bce = nn.functional.binary_cross_entropy(
                scores.clamp(1e-7, 1 - 1e-7), targets, reduction="none")
            seq_loss = (bce * mask).sum(1) / mask.sum(1).clamp(min=1)
            is_fail = (succ == 0).float()
            is_succ = (succ == 1).float()
            weighted = seq_loss * is_fail * w_fail + seq_loss * is_succ * w_succ
            loss = weighted.sum() / B
        ...
        if epoch % EVAL_EVERY == 0:
            ...
            with torch.no_grad():
                for x, lab, succ, lens in va_ld:
                    ...
                    ep_sc.extend(sc.masked_fill(~m, -1).max(1).values.cpu().tolist())
...
def main():
    ...
    print(f"=== SAFE-LSTM | split={args.split} seed={args.seed} ===")
