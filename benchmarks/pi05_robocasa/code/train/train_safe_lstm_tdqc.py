"""Train SAFE-LSTM-TDQC strict (paper-faithful TD(0) on hidden_states).

Architecture: SafeLSTM (same as plain SAFE-LSTM, sigmoid output).
Hyperparameters (paper sweep lstm_TD0):
  Adam lr=1e-4, wd=0, batch=64, 1000 epochs (early stop)
  StepLR step_size=200 gamma=0.8
  TDLoss + target network (sync every 10 batches)

Usage:
  source env.sh
  python code/train/train_safe_lstm_tdqc.py --split allseen --seed 0
"""
import argparse
import random
import sys
...
CKPT_DIR = ROOT / "checkpoints" / "safe_lstm_tdqc"
...
def train(splits, seed):
    ...
    main = SafeLSTM(input_dim=1024, hidden=256).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % EVAL_EVERY == 0:
            ...
            if epoch % 100 == 0 or no_improve == 0:
...
def main_cli():
    ...
    print(f"=== SAFE-LSTM-TDQC-strict | split={args.split} seed={args.seed} ===")
