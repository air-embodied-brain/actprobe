"""
Train ActProbe (= ActProbe, 2-feat) for OpenVLA+LIBERO.

Reproduces the 6 ckpts in checkpoints/actprobe/ (3 seeds × 2 splits).
Implements the 2-feature ActProbeNet from paper §3, adapted to use lib/.

Usage:
  source env.sh
  python code/train/train_actprobe.py --seeds 0 1 2 --splits allseen unseen
  # default --out: $OPENVLA_LIBERO_CKPT/actprobe/{split}_seed{N}.pt
"""
import argparse
import random
import sys
...
# Hyperparams for the paper-faithful 2-feature setting.
BATCH_SIZE = 64
LR         = 1e-3
WEIGHT_DEC = 1e-4
T_MAX      = 400
PATIENCE   = 50    # in 5-epoch units → 250 actual epochs without improvement
N_EPOCHS   = 400
SUBSAMPLE_SEED = 42
...
class WDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ts   = (np.arange(T, dtype=np.float32) / 100.0).reshape(-1, 1)  # absolute t/100 (length-leak-free)
...
def train_one(seed, split_type, eps_metrics, task_embs, ckpt_path):
    ...
    sub = {tid: [{**e, "raw_full": e["raw"], "raw": e["raw"][:, PAPER_FEAT_IDX].astype(np.float32)}
                  for e in eps]
    ...
    model = ActProbeNet(n_feat=len(PAPER_FEAT_IDX)).to(DEVICE)
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if epoch % 5 == 0:
            ...
            if auc > best_auc + 1e-4:
                ...
                no_imp += 5
            if epoch % 50 == 0:
                print(f"      ep{epoch:3d}  val_q025taskmax={auc:.4f}  best={best_auc:.4f}  patience={no_imp}/{PATIENCE * 5}")
...
def main():
    ...
    eps_metrics      = data_mod.subsample_50eps(eps_metrics_full, seed=SUBSAMPLE_SEED)
