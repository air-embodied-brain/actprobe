...
- 10 task 全用，**无 EXCLUDE_TASKS**
- N_UNSEEN = 3 (vs GR00T 7)
"""
import random

import numpy as np

# Unseen split: 3 unseen / 7 seen, randomized per-seed (matches paper)
N_UNSEEN = 3
TASK_SPLIT_SEED_OFFSET = 1000
...
def split_unseen(task_eps, seed):
    """Unseen Protocol (Paper §2b): N_UNSEEN tasks held out as test.

    For seen tasks: 70% train, 30% val (no test).
    For unseen tasks: all eps → test.
    """
    rng_task = random.Random(seed + TASK_SPLIT_SEED_OFFSET)
    all_tasks = sorted(task_eps.keys())
    rng_task.shuffle(all_tasks)
    unseen = all_tasks[:N_UNSEEN]
    seen   = all_tasks[N_UNSEEN:]

    splits = {"train": [], "val": [], "test": [],
              "_unseen_tasks": unseen, "_seen_tasks": seen}
    rng = random.Random(seed)
    for tid in seen:
        for lv in [0, 1]:
            grp = [e for e in task_eps[tid] if e["label"] == lv]
...
def fit_norm(eps, key="raw"):
    ...
    Args:
      eps: list of dicts with the given key
      key: name of array field to normalize over (e.g., "raw" for 10feat, "raw_hs" for HS)
...
def compute_task_min(eps):
    ...
    Pass test-split episodes for unseen protocol; the train split does not
    contain unseen tasks, so train-derived lookups would fall back incorrectly.
    """
