...
def load_ckpt(path, device="cuda:0"):
    ...
    # Handle checkpoints that use lstm.* keys vs rnn.* keys.
    # Our SafeLSTM uses self.rnn, so remap lstm.* → rnn.*.
    if any(k.startswith("lstm.") for k in sd):
        sd = {k.replace("lstm.", "rnn.", 1) if k.startswith("lstm.") else k: v
              for k, v in sd.items()}
    model = SafeLSTM(input_dim=4096, hidden=256).to(device)
