...
Pi0.5 adaptation: lang_emb is keyed on ep['instruction'] (236 unique
instructions across 24 tasks), not ep['task_id'].
...
def subset_2feat(task_eps, idx=PAPER_FEAT_IDX):
    """Return new task_eps with raw subset to 2 feats. Original 10-feat preserved as raw10."""
    out = {}
...
@torch.no_grad()
def score_episodes(model, episodes, qwen3_embs, norm_mean, norm_std, device="cuda:0"):
    """Run ActProbe. lang_emb keyed by ep['instruction'] (Pi0.5 convention)."""
    results = []
    for ep in episodes:
        raw = ep["raw"]
        T = raw.shape[0]
        ...
        lang = torch.from_numpy(qwen3_embs[ep["instruction"]].astype(np.float32)).unsqueeze(0).to(device)
